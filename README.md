...
| `nix run .#mhf-debug-build` | `mhf-debug-build` | Build the standalone offline debug launcher |
| `nix run .#mhf-debug-launcher` | `mhf-debug-launcher` | Build and start the offline test quest |
...
Only `mhf-launcher` and `mhf-debug-launcher` prepare the writable MHF configuration and default `WINEPREFIX`.
...
The MHF launcher writes game settings back to `$PROJECT_STATE/config/mhf.toml`
(`.state/config/mhf.toml` by default). The adjacent `mhf.generated.toml` stores
the last original configuration generated by Nix. Launches compare the generated
content with that snapshot and reuse the writable file when the content matches.
When the generated content changes, both files are refreshed, resetting game
settings previously written back to the writable file. These fixed filenames do
not accumulate configuration history. Export `MHF_CONFIG` to use a separately
