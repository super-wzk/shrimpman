...
mhf_executable := justfile_directory() + "/target/" + mhf_target + "/release/mhf-launcher.exe"
mhf_cargo := if os() == "windows" { "cargo" } else { "PATH=\"" + justfile_directory() + "/crates/mhf-launcher/tools:$PATH\" RANLIB_i686_pc_windows_msvc=llvm-ranlib cargo xwin" }
...
mhf-build:
    {{ mhf_cargo }} build -p shrimpman-mhf-launcher --release --target {{ mhf_target }} {{ mhf_xwin_args }}
