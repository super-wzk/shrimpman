...
pub(super) fn generate() -> Result<(), String> {
    ...
    let (generated_rust, generated_binary) = compile_dictionary(&locales, &layouts)?;
    fs::write(
...
fn expand_resource_layout(
    ...
    let (id, identity, runtime, body) = match definition {
        ResourceDefinition::Records {
            ...
            (id, identity, runtime, ResourceBody::Records(tables))
        }
        ResourceDefinition::Quest {
            ...
            (id, identity, runtime, ResourceBody::Quest(quest))
        }
    };
...
fn expand_record_tables(
    ...
    tables.sort_by_key(|table| table.root);
    Ok(tables)
}
...
fn validate_record_tables(
    ...
    for table in tables {
        ...
        if !table.root.is_multiple_of(4)
            || previous_root.is_some_and(|previous| previous >= table.root)
        {
            return Err(format!(
                "{} resource {:?} table roots must be aligned and strictly ordered",
                path.display(),
                resource.id
            ));
        }
        previous_root = Some(table.root);
        if table.records == 0 {
            return Err(format!(
                "{} resource {:?} has an invalid records table at root {}",
                path.display(),
                resource.id,
                table.root
            ));
...
fn group_width(layouts: &[ResourceLayout], key: &TranslationGroupKey) -> Result<u16, String> {
    ...
    match &resource.body {
        ResourceBody::Records(tables) => {
            ...
            if record_id >= table.records {
                return Err(format!(
                    "key {key} is outside the table's {}-record layout",
                    table.records
                ));
            }
            Ok(table.parts)
...
fn compile_dictionary(
    ...
    output.push('\n');
    render_resource_layouts(&mut output, layouts, &group_ids);
    render_main_resource_hooks(&mut output, layouts);
    writeln!(
        output,
...
fn render_resource_layouts(
    ...
    for (resource_index, resource) in layouts.iter().enumerate() {
        ...
        for table in tables {
            writeln!(
                output,
                "    RecordTableLayout {{ id: {:?}, translation_group: {}, root: 0x{:08X}, records: {}, text_offset: {}, parts: {}, stride: {} }},",
                table.id,
    ...
    for (resource_index, resource) in layouts.iter().enumerate() {
        ...
        writeln!(
            output,
            "    ResourceLayout {{ id: {:?}, magic: 0x{:08X}, format_version: {}, body: {body} }},",
            resource.id, resource.magic, resource.format_version
        )
        .expect("writing to String cannot fail");
    }
    output.push_str("];\n\n");
