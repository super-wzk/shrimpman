...
pub(crate) struct HookState {
    // Drop the retained DLL before data it could inspect from DllMain.
    _module: ModuleReference,
    pub(super) module_base: usize,
    glyphs: Mutex<rendering::GlyphCache>,
    gdi: gdi::RenderingHooks,
}
...
pub(crate) unsafe fn install(
    ...
    .map_err(|error| format!("failed to retain native UTF-8 text module: {error}"))?;
    let retained = ModuleReference(retained.0 as usize);
    for entry in entries {
        let original =
    ...
    let gdi = unsafe { gdi::create_hooks(&mut hooks, font_name) }?;
    unsafe {
...
mod tests {
    ...
    fn supported_client_installs_and_removes_text_hooks() {
        ...
        let retained = super::ModuleReference(module.0 as usize);
        for _ in 0..2 {
            let mut localization = unsafe { crate::localization::install(module, None) }
                .expect("install native and resource localization");
            let mut hooks = unsafe { super::install(module, c"Arial".to_bytes_with_nul()) }
