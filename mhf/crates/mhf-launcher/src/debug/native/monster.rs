...
pub(super) unsafe fn release(state: &State, runtime: &mut Runtime) {
    ...
    state
        .session
        .control
        .set_monster_input(MonsterInput::default());
}
    ...
    unsafe {
        ...
        let quest =
            state
                .session
                .quest
                .with_monster(species, current.area, current.position, yaw)?;
        ...
        runtime.monster = Some(Control {
            ...
            instance: None,
            spawn_offset: quest.spawn_offset,
            area: current.area,
        ...
        runtime.quest_override = Some(quest.bytes);
...
pub(super) unsafe fn before_frame(state: &State, runtime: &mut Runtime) {
    unsafe {
        ...
        let input = state.session.control.monster_input();
        let control = runtime.monster.as_mut().unwrap();
        let now = Instant::now();
    ...
    unsafe {
            ...
            .is_some_and(|control| control.waiting)
        {
            ...
            if control.instance.is_none() && pool != 0 && player.is_some() {
                ...
                if let (Some(resource), Some(bytes)) = (resource, &runtime.quest_override)
                    && buffer != 0
                    && control.spawn_offset + 60 <= bytes.len()
                {
