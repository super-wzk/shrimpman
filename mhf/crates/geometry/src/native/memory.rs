...
use windows::{
    Win32::{
        ...
        System::{
            ...
            LibraryLoader::{GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, GetModuleHandleExA},
...
pub(super) struct Module(pub usize);

impl Module {
    pub unsafe fn retain(module: HMODULE) -> Result<Self, String> {
        ...
        .map_err(|e| format!("retain geometry module: {e}"))?;
...
impl CodePatches {
    pub unsafe fn install(base: usize) -> Result<Self, String> {
        let mut guard = Self { base, applied: 0 };
        for patch in PATCHES {
