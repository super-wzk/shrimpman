            ...
            (
                ...
                (
                    ...
                    credential_error.map(|error| {
                        eprintln!("{error}");
                        Effect::NotifyError("登录成功，但无法更新已保存的密码。".into())
                    }),
                )
            }
            (
...
fn sign_error_message(error: &sign::Error) -> String {
    ...
    match error {
        sign::Error::Timeout | sign::Error::Http(ureq::Error::Timeout(_)) => {
            return format!(
                "请求超时（最长等待 {} 秒），请检查网络后重试。",
                sign::REQUEST_TIMEOUT.as_secs(),
            );
        }
        sign::Error::Http(_) | sign::Error::Tcp(_) => {
            return "无法连接登录服务，请检查网络后重试。".to_owned();
        }
        sign::Error::InvalidResponse(_) => {
            return "登录服务返回了无效响应，请重试。".to_owned();
        }
        _ => {}
    }
    match error.code() {
        Some("wrong_password") => "用户名或密码错误。".to_owned(),
        Some("illegal_input") => "用户名或密码格式无效。".to_owned(),
        Some("invalid_session") => "登录会话已过期，请重新登录。".to_owned(),
        Some("pending_character_exists") => "已有一个新角色等待完成创建。".to_owned(),
        Some("character_not_found") => "该角色已不存在。".to_owned(),
        Some("internal_error") => "登录服务无法处理请求，请稍后重试。".to_owned(),
        _ => error.to_string(),
    }
}
