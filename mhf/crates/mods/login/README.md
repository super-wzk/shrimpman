...
HTTP/HTTPS 可以包含 API 基础路径。界面保存地址时校验协议和地址。
地址缺失或无效时直接打开登录服务器设置，修正并保存后即可登录。

登录页和角色页的“设置”菜单提供三个独立编辑页：

- 字体：游戏字体名称、内置字体、字重和渲染质量。
- 屏幕：窗口／全屏模式，以及各自的常用或自定义分辨率。
- 登录服务器：服务地址，保留现有 `encoding` 配置，不提供编码控件。

每页保存后写回当前使用的 `mhf.toml`，保留其他节及未知字段；取消不写入。
字体和屏幕设置在本次游戏启动时生效。更换服务器后清除当前会话及表单，重新读取
新服务器对应的系统凭据；地址未变化时保留当前登录状态。请求进行中无法打开设置。
`MHF_SIGN__ENDPOINT` 仍优先于文件，此时界面显示覆盖来源并禁用地址修改和保存。
