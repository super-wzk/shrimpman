...
//! and candidate UI are shared with egui through `mhf-ui`.
...
use mhf_ui::{HostIme, HostImeTarget, InputCaptureState};
...
pub(crate) struct GameIme {
    editor: Editor,
    module: ModuleReference,
}

impl GameIme {
    /// Call after the overlay and native IME callbacks have stopped. The host
    /// retains its own DLL reference until every native state is ready to release.
    ...
    pub(crate) unsafe fn new(module: HMODULE) -> Result<Arc<Self>, String> {
