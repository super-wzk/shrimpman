use super::{
    HookState, Key, MemoryRange, QuestTableLayout, RESOURCE_LAYOUTS, RecordCount,
    RecordTableLayout, ResourceBodyLayout, TextArena, replacement_for_key,
};
        ...
        for part in 0..layout.parts {
            ...
            unsafe {
                patch_cell(
                    ...
                    text,
                    image,
                    Key::resource(resource_id, layout.id, record, part),
                    cell,
                    code_page,
                )
            };
                ...
                for part in 0..layout.parts {
                    ...
                    unsafe {
                        patch_cell(
                            ...
                            text,
                            image,
                            Key::resource(resource_id, layout.id, u32::from(quest_id), part),
                            cell,
                            code_page,
                        )
                    };
...
fn keyconfig_header(key: Key, header: &CStr) -> Option<Vec<u8>> {
    ...
    if key != Key::resource("mhfpac", "table_793", 0, 0) {
        return None;
    }
    let body = header
        .to_bytes_with_nul()
...
mod tests {
    ...
    fn table_segments_keep_their_own_zero_based_translation_ids() {
        let provider = TranslationService::with_locale(None, MissingTranslation::Key);
        let mut state = test_state();
        state.translation = Some(unsafe { mhf_translation::bind(provider.api()) });
    ...
    fn headerless_resources_use_fixed_encoding_and_the_shared_translation_policy() {
        for (locale, missing, expected) in [
            ...
            let provider = TranslationService::with_locale(locale, missing);
            let mut state = test_state();
            state.translation = Some(unsafe { mhf_translation::bind(provider.api()) });
