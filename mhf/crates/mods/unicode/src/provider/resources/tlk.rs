use super::{
    CodeHook, HOOK_STATE, HookState, Key, MemoryRange, replacement_for_key,
        ...
        for record in 0..record_count {
            ...
            let replacement = if stage != UNKNOWN_STAGE && section.first {
                replacement_for_key(
                    state,
                    &mut text,
                    Key::stage(stage, section.id, record as u16),
                    source,
                    code_page,
                )
            } else {
...
mod tests {
    ...
    fn converts_every_record_with_sparse_overrides_and_preserves_utf8_on_repeat() {
        let provider = TranslationService::with_locale(Some("test"), MissingTranslation::Original);
    ...
    fn converts_originals_when_stage_is_unknown_without_applying_missing_policy() {
        let provider = TranslationService::with_locale(Some("test"), MissingTranslation::Key);
        let mut state = test_state();
        state.translation = Some(unsafe { mhf_translation::bind(provider.api()) });
