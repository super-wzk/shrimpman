...
use mhf_translation::Translation;

fn key(group: &'static str, record_id: u32) -> Key<'static> {
    Key::resource("native", group, record_id, 0)
}
...
pub(super) unsafe fn install(
    ...
    let mut resolve =
        |group, id, source: &CStr| replacement(translation, &mut text, key(group, id), source, 932);
...
impl NativeTextGuard {
    /// Restore native pointers and surrender the module reference, retaining the
    /// arena until the host has unloaded the game and drops the retired state.
    pub(super) unsafe fn prepare_release(&mut self) -> Result<(), String> {
        unsafe { self.restore() }?;
...
mod tests {
    ...
    fn native_keys_share_locale_selection_and_missing_policy() {
        use crate::provider::resources::test_translation::TestTranslation as TranslationService;
        use mhf_translation::MissingTranslation;

        let locale = "ja-JP";
        let provider = TranslationService::with_locale(Some(locale), MissingTranslation::Empty);
        let translation = Some(unsafe { mhf_translation::bind(provider.api()) });
        let mut arena = TextArena::default();
        let original = c"\x82\xc6";
        for (group, id, expected) in [("rank", 4, "ＧＲ"), ("room", 0, "空き")] {
            let pointer =
                replacement(translation, &mut arena, key(group, id), original, 932).unwrap();
            assert_eq!(
        ...
        for (missing, expected) in [
            ...
            let provider = TranslationService::with_locale(None, missing);
            let pointer = replacement(
                Some(unsafe { mhf_translation::bind(provider.api()) }),
                &mut arena,
                key("rank", 0),
                original,
                932,
    ...
    #[test]
    fn preparing_module_release_restores_pointers_but_keeps_text_until_state_drop() {
        let page = Page::new();
        let arena = TextArena::default();
        let replacement = arena
            .storage
            .alloc_slice_copy(c"退出时仍然有效".to_bytes_with_nul())
            .as_ptr();
        let address = unsafe { page.0.add(0x40) }.cast::<u32>();
        let original = unsafe { page.0.add(0x80) } as u32;
        unsafe { ptr::write_unaligned(address, original) };
        ...
        // A failed restoration preserves the arena and the patch for retry.
        unsafe { ptr::write_unaligned(address, 0x1234) };
        assert!(unsafe { guard.prepare_release() }.is_err());
        assert!(guard.text.is_some());
        unsafe { ptr::write_unaligned(address, replacement as u32) };
        unsafe { guard.prepare_release() }.unwrap();
        assert_eq!(unsafe { ptr::read_unaligned(address) }, original);
        assert!(guard.text.is_some());
        assert_eq!(
            unsafe { CStr::from_ptr(replacement.cast()) },
            c"退出时仍然有效"
        );

        // Model the host unloading its image before dropping retired state.
        drop(page);
        drop(guard);
    }
