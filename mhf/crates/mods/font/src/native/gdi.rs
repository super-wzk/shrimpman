...
pub struct TextRenderer {
    pub measure: GetTextExtentPoint32AFn,
    pub draw: ExtTextOutAFn,
}
...
///
/// # Safety
/// Optional renderer callbacks must implement the declared GDI calling
/// conventions and remain callable until native text callers have stopped and
/// the returned hook group has been uninstalled.
pub unsafe fn install_game(
    font_name: &[u8],
    renderer: Option<TextRenderer>,
...
///
/// # Safety
/// `hdc` must be a live device context, accessed on its owning native thread.
pub unsafe fn corrected_y(hdc: HDC, y: i32) -> i32 {
    let invocation = HOOK_STATE.enter();
    invocation
        .state()
