...
use crate::backend::window::{DummyWindow, WindowBinding, WindowState};
use crate::backend::{Error, HostIme, InputCaptureState, Overlay, Result};
...
pub struct D3d9Hook {
    hooks: Option<HookGuard<HookState>>,
...
impl D3d9Hook {
    ...
    /// # Errors
    ///
    /// Returns an error if either hook cannot be disabled or removed. The
    /// handle retains its remaining hooks and resources so cleanup can retry.
    pub fn uninstall(&mut self) -> Result<()> {
        self.capture.reset();
        if let Some(hooks) = &mut self.hooks {
            hooks.uninstall().map_err(Error::new)?;
            self.hooks = None;
        }
        Ok(())
    }
