...
fn compile_dictionary(
    ...
    let mut output =
        "// @generated by build.rs from the resource layout and locales/*.jsonl. Do not edit.\n\n"
            .to_owned();
    ...
    output.push('\n');
    // Resource ordinals are local to this catalog build. Keep their stable
    // textual identity so independent Mod versions need not share ordinals.
    let groups: BTreeMap<_, _> = locales
        .iter()
        .flat_map(|locale| &locale.translations)
        .filter_map(|translation| match &translation.key.group {
            TranslationGroupKey::Resource {
                resource_id,
                group_id,
                ..
            } => Some((
                (resource_id.as_str(), group_id.as_str()),
                catalog.group_id(resource_id, group_id),
            )),
            TranslationGroupKey::Stage { .. } => None,
        })
        .collect();
    output.push_str("static TRANSLATION_GROUPS: &[(&str, &str, u32)] = &[\n");
    for ((resource, group), id) in groups {
        writeln!(output, "    ({resource:?}, {group:?}, {id}),")
            .expect("writing to String cannot fail");
    }
    output.push_str("];\n\n");
    writeln!(
