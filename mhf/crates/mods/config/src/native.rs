...
"#;
        ...
        let store = Store::load(config_path.clone()).unwrap();
        let mut hooks = install(&ini_name, Arc::new(Mutex::new(store))).unwrap();
        assert_eq!(read(), 42);
        assert_eq!(
            unsafe {
        ...
        let store = Store::load(config_path).unwrap();
        let hooks = install(&ini_name, Arc::new(Mutex::new(store))).unwrap();
