...
| --- | ---: | ---: |
| `15/3e/57`（count；首分支另带 be_u16） | 3 | 7 |
| `1c/1d/20/23/27/2c/33/70/73/75/76/7a/7d`（count；首分支另带 u8） | 3 | 6 |
| `79`（count + callback argument；首分支另带 u8） | 4 | 7 |
| `80`（count；首分支另带 weight） | 3 | 6 |
| `83`（count；首分支无参数） | 3 | 5 |
| `94`（count；首分支另带 u8） | 3 | 5 |

证据：`10861BB0`、`10863A40`、`10865910`、`10863220` 等 list handlers
读 count 后跨过两字节标记，再读取比较参数；`10862D20` 多读 callback argument。
`10866E50` 和 `10867460` 从三字节头后查找/跳过分支标记。
`94` 尤其不能照抄 skip：`108682B0` 实际消费首分支参数，而 skip 停在该参数上。
Rathian `11854D88` 的 `13 94 00 02 94 01 00 70 ...` 中，`11854D8E` 的
`00` 是 `94/1` 参数，不是 default stop。结构校验仍拒绝块内真正的 stop、
缺失首分支标记、错配和未闭合的块。
...
| 选择子 | 总宽度 |
| --- | ---: |
| `0` | 3 字节（`opcode + selector + count`；后跟独立的 `15/1` 指令） |
| `1` | 4 字节 |
| `2`、`3` | 2 字节 |

未知选择子保留旧 codec 的宽度记录（不保证与执行/skip 一致），但结构校验拒绝
安装这些未确认的形式，例如：
