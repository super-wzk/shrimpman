...
mod tests {
    ...
    use crate::{
        MissingTranslation,
        translation::{CompiledDictionary, CompiledLocale, Translations},
    };
    use std::ffi::CStr;
    ...
    fn converts_every_record_with_sparse_overrides_and_preserves_utf8_on_repeat() {
        ...
        state.translations =
            Translations::with_locale(DICTIONARY.locale("test"), MissingTranslation::Original);
    ...
    fn converts_originals_when_stage_is_unknown_without_applying_missing_policy() {
        let mut state = test_state();
        state.translations =
            Translations::with_locale(DICTIONARY.locale("test"), MissingTranslation::Key);
