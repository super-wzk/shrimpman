...
mod tests {
    ...
    fn native_keys_share_locale_selection_and_missing_policy() {
        use crate::translation::MissingTranslation;

        let locale = crate::translation::test_locale("ja-JP");
        let translations = Translations::with_locale(Some(locale), MissingTranslation::Empty);
        let mut arena = TextArena::default();
        let original = c"\x82\xc6";
        for (group, id, expected) in [("rank", 4, "ＧＲ"), ("room", 0, "空き")] {
            let pointer =
                replacement(&translations, &mut arena, key(group, id), original, 932).unwrap();
            assert_eq!(
        ...
        for (missing, expected) in [
            ...
            let pointer = replacement(
                &Translations::with_locale(None, missing),
                &mut arena,
                key("rank", 0),
                original,
                932,
