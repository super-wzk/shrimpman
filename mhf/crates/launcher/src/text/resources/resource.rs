...
mod tests {
    ...
    use crate::translation::{MissingTranslation, Translations};
    ...
    fn table_segments_keep_their_own_zero_based_translation_ids() {
        ...
        state.translations = Translations::with_locale(None, MissingTranslation::Key);
    ...
    fn headerless_resources_use_fixed_encoding_and_the_shared_translation_policy() {
        for (locale, missing, expected) in [
            ...
            let locale = locale.map(crate::translation::test_locale);
            let mut state = test_state();
            state.translations = Translations::with_locale(locale, missing);
