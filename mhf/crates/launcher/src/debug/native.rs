...
pub(crate) struct State {
    // Release the DLL before native-facing buffers it could inspect in DllMain.
...
impl ModuleReference {
    unsafe fn acquire(module: HMODULE) -> Result<Self, String> {
        ...
        .map_err(|error| format!("failed to retain the debug game module: {error}"))?;
...
pub(crate) unsafe fn install(
    ...
    session.validate_module(module)?;
...
unsafe fn snapshot(state: &State, runtime: &Runtime) -> DebugSnapshot {
    let mut snapshot = DebugSnapshot {
        quest_id: state.session.quest_id(),
            ...
            for command in state.control.commands() {
                ...
                let result = match command {
                        ...
                        if runtime.monster.is_some() && (current.ready || current.scene == 5) =>
                    {
                        ...
                        state.session.reset_quest();
                        runtime.pending_action = None;
                        restart(state, &mut runtime).map(|()| "正在恢复猎人与原始任务".into())
                    }
...
mod text_tests {
    ...
    #[cfg(not(feature = "unicode"))]
    #[test]
    fn catalog_decodes_original_cp932_without_unicode_text() {
        assert_eq!(
    ...
    #[cfg(feature = "unicode")]
    #[test]
    fn catalog_decodes_utf8_with_unicode_text() {
        assert_eq!(
