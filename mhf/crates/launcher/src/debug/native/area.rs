...
pub(super) unsafe fn enter_at_position(
    ...
    unsafe {
        ...
        for record in exit_records(state, area) {
...
pub(super) unsafe fn change(state: &State, destination: u16) -> Result<String, String> {
    unsafe {
        ...
        for source in areas {
            for record in exit_records(state, source) {
...
unsafe fn exit_records(state: &State, area: u16) -> Vec<usize> {
    unsafe {
        ...
        let size = state.session.quest_len();
