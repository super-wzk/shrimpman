...
pub(crate) fn launch(
    ...
    let (settings, _translation) = match &mode {
        ...
        LaunchMode::Offline {
            ...
            if settings.video.graphics_version != GraphicsVersion::HighDefinition {
                return Err("离线运行目前需要 HD 客户端".into());
            }
    ...
    #[cfg(feature = "unicode")]
    let mut resources = unsafe { crate::text::resources::install(game.handle(), _translation) }?;
    data.mhfo_module = game.handle();
    data.mhfo_main = Some(entry);
    #[cfg(feature = "offline")]
    let (session, _debug_enabled) = match mode {
        #[cfg(feature = "login")]
        LaunchMode::Online(_) => (None, false),
        LaunchMode::Offline { session, debug, .. } => (Some(session), debug),
    };
    #[cfg(feature = "offline")]
    let mut offline = session
        .as_ref()
        .map(|session| unsafe { crate::offline::install(game.handle(), session.clone()) })
        .transpose()?;
    #[cfg(feature = "debug")]
    let control = _debug_enabled.then(crate::debug::DebugControl::new);
    #[cfg(feature = "debug")]
    let mut debug = session
        .as_ref()
        .zip(control.as_ref())
        .map(|(session, control)| unsafe {
            crate::debug::install(game.handle(), session.clone(), control.clone())
        })
        .transpose()?;
    let overlay = unsafe {
    ...
    #[cfg(feature = "unicode")]
    let renderer = Some(crate::text::gdi_renderer());
    #[cfg(not(feature = "unicode"))]
    let renderer = None;
    let mut font = unsafe { crate::font::install_game(&data.params.font_name, renderer) }?;
    #[cfg(feature = "unicode")]
    let mut text = unsafe { crate::text::install(game.handle()) }?;
    let code = unsafe { entry(&mut data.params) };

    // Stop native text callers before removing the font hooks they use. Resource
    // guards retain the DLL and text buffers through cleanup, including DllMain.
    let overlay_cleanup = overlay.uninstall().map_err(|error| error.to_string());
    #[cfg(feature = "unicode")]
    let text_cleanup = text.uninstall();
    let font_cleanup = font.uninstall();
    #[cfg(feature = "debug")]
    let debug_cleanup = debug.as_mut().map(|debug| debug.uninstall()).transpose();
    #[cfg(feature = "offline")]
    let offline_cleanup = offline
        .as_mut()
        .map(|offline| offline.uninstall())
        .transpose();
    let geometry_cleanup = geometry
        ...
        .transpose();
    #[cfg(feature = "unicode")]
    let resources_cleanup = resources.uninstall();
    drop(game);
    let errors = [
        overlay_cleanup.err(),
        #[cfg(feature = "unicode")]
