...
#[path = "../src/text/resources/native/layout.rs"]
mod native_layout;
...
pub(super) fn generate(
    ...
    println!("cargo:rerun-if-changed=src/text/resources/native/layout.rs");
    let layouts = read_resource_layouts(&layout_path)?;
    let group_ids = resource_group_ids(&layouts)?;
    let catalog = ResourceCatalog { layouts, group_ids };

    let mut resources =
        "// @generated by build.rs from the resource layout. Do not edit.\n\n".to_owned();
    render_resource_layouts(&mut resources, &catalog.layouts, &catalog.group_ids);
