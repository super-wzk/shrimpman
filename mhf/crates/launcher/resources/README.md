...
`unicode` Cargo feature 负责资源和 DLL 原生文本转码、Unicode 字符边界与输入编辑，
以及 Unicode 绘制。它可以独立于 `translation` 启用，所有原文按明确的源代码页转为 UTF-8。
字体资源、注册和度量修正由 `font` 模块负责，也可用于游戏原始编码。

构建脚本读取 `layout.json`，结合 `src/text/resources/native/layout.rs` 中的 DLL 文本布局，
生成 `resources.rs`。资源布局和稳定 group ID 由 `build/resource_layout.rs` 统一处理；
只有启用 `translation` 时，`build/translation_dictionary.rs` 才会读取 locale JSONL 并额外生成
`translations.rs` 与 `translations.bin`。两者共享同一次解析的资源布局和 group ID。
