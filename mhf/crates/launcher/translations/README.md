...
`translation` Cargo feature 默认启用，负责 JSONL 读取、词典生成与运行时译文查询，并依赖
`unicode` 提供资源转码、Unicode 字符处理和绘制。字体资源与字体度量修正由 `font`
模块负责，不依赖这两个 feature。

使用 `--no-default-features --features login,unicode` 或
`--no-default-features --features debug,unicode` 可只启用 Unicode 文本能力：构建时读取
[`../resources/layout.json`](../resources/layout.json) 并生成资源布局，不读取任何 locale JSONL。
省略 `unicode` 时，资源与 DLL 文本保留游戏原生编码。
禁用 `translation` 后已有 `[translation]` 配置会被忽略，游戏回写其他设置时保留该配置。
...
资源 schema、运行时加载边界和转码规则见 [资源布局说明](../resources/README.md)。
...
其他输出目录。游戏资源只在执行生成器时需要；启用 `unicode` 的 `build.rs` 只需读取
`resources/layout.json`；启用 `translation` 时才额外读取 `translations/*.jsonl`。
...
关闭 `translation` feature 后，`unicode` 仍可独立启用并转换原文。只有关闭
`unicode` 时才停止资源和 DLL 文本转码，游戏保留原生文本处理。
...
against `src/text/resources/native/layout.rs`, which is shared with the native
