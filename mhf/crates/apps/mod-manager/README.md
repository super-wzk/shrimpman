...
保存和导出时解析配置中明确启用的 Mod 及其声明依赖。启动器默认启用 Login，
Base 由 Login、Debug 等 Mod 的声明依赖按需带入，最终加载顺序由依赖关系确定。
管理器不加载 Mod DLL 或游戏。
...
- 设置「自动／启用／禁用」，在「指定版本」中输入 semver 范围或选择已安装的精确版本；「自动」在被其他 Mod 依赖或默认启动规则需要时启用。
- 预览依赖解析结果与冲突，保存设置或撤销修改。保存保留原 TOML 的其他字段与注释，下次启动游戏生效。
- 依赖异常按实际启动组合中的 Mod 标记：检查明确启用、默认启动规则带入的 Mod 及其依赖；未被需要的「自动」Mod 不检查依赖。异常项显示红色警告，详情在对应依赖旁说明缺包、被禁用或版本冲突。主动禁用的 Mod 不因被其他项依赖而标红；正常 Mod 不显示其他项的错误。阻止保存或导出的全局失败摘要位于底部保存区。
...
- `export <ZIP> [ID[@范围] ...]` 先解析已安装包的依赖，导出精确版本及完整依赖。省略 ID 时以配置中明确启用的 Mod 为根；显式 ID 列表替换本次导出的根选择，保留依赖的配置版本约束和明确禁用状态。命令行选择不写回配置。输出文件必须尚不存在。
