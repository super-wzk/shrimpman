use egui::{
    Align, Color32, FontSelection, Layout, Rect, Response, RichText, Sense, Shape, Stroke,
...
pub fn notice(ui: &mut Ui, kind: NoticeKind, text: &str) -> Response {
    ...
    let (icon, accent) = match kind {
        NoticeKind::Success => (Icon::Check, tokens.success),
        NoticeKind::Warning => (Icon::Warning, ui.visuals().warn_fg_color),
        NoticeKind::Danger => (Icon::Warning, ui.visuals().error_fg_color),
    };
...
impl Widget for Meter<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let color = self.color.unwrap_or(ui.visuals().selection.bg_fill);
        ...
        if ui.is_rect_visible(rect) {
            ...
            if fill.width() > 0.0 {
                painter.add(paint::chamfer(fill, 2.0, color, Stroke::NONE));
            }
        }
        response
    }
