...
/// A keyboard-accessible button or selectable row. Use `Ui::push_id` for
...
fn choice(ui: &mut Ui, checked: &mut bool, label: &str, switch: bool) -> Response {
    ...
    if ui.is_rect_visible(rect) {
        ...
        let tokens = crate::Tokens::get(ui);
        let mut visuals = paint::visuals(ui, &response, false);
        if *checked {
            visuals.bg_fill =
                paint::button_visuals(ui, &response, false, ButtonKind::Primary).bg_fill;
            visuals.bg_stroke = Stroke::NONE;
        }
        visuals.corner_radius = egui::CornerRadius::same(if switch { 10 } else { 5 });
        if paint::focused(ui, &response) {
            visuals.bg_stroke = paint::focus_stroke(ui, *checked);
        }
        ...
        if switch {
            ...
            painter.circle_filled(
                ...
                ui.spacing().icon_width_inner * 0.5,
                mark_color,
            );
        } else if *checked {
            Icon::Check.paint(
