...
| 0.34 ScrollArea 边缘淡出、UiStack 背景 | Panel 使用原生圆角 Frame 登记真实表面底色，原生淡出能识别 Panel 和 Raised 层级。 |
...
| 0.34 原生 Panel、0.35/0.36 面板拖动开关 | 原生 `egui::Panel` 用于屏幕边缘占位、缩放和折叠；本库 Panel 是带标题与圆角的内容表面。布局直接使用原生 API，内容表面保留为设计系统组件。 |
| 多 Viewport | 额外 OS 窗口需要 backend 实现创建、输入和渲染。当前游戏宿主使用 egui 的嵌入回退；游戏内 Window 不等于 OS Viewport。 |
| 0.35 Window 标题 Atoms、0.36 `title_frame` | Panel 已使用原生 Frame；Window 继续薄层组合标题、主题关闭按钮和内容，位置、缩放和生命周期由原生 Window 执行。 |
...
| 自定义 Widget / epaint | 圆角控件、图标和选中勾选由主题统一绘制。按钮和输入控件聚焦时将原有内部边框加粗至 2px，金色填充使用深色边框；Tabs 和 ScrollPanel 保留组件自身的内部焦点框。保留主操作颜色、选中填充与勾选，不扩展矩形或增加布局占位。绘制使用原生 Sense、Response、Ui 作用域、Painter、Shape 和字体栅格化，不改变焦点或 Tab 顺序。 |
...
Modal 的输入隔离只作用于 egui。需要独占游戏输入的界面应由 Overlay 宿主持续阻断
鼠标和键盘，关闭后恢复穿透；控件短暂失焦或鼠标离开面板不应提前解除阻断。
