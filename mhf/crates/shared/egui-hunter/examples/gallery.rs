...
impl Gallery {
    ...
    fn header(&self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            Icon::Sword.paint(ui.painter(), rect.shrink(4.0), ui.visuals().hyperlink_color);
            ui.heading("猎人工坊");
            if wide {
                ui.add_space(24.0);
                self.palette(ui);
    ...
    fn palette(&self, ui: &mut egui::Ui) {
        ui.horizontal_wrapped(|ui| {
            for (name, color) in [
                ("正文", ui.visuals().text_color()),
                ("面板", ui.visuals().window_fill),
                ("选中", ui.visuals().selection.stroke.color),
                ("焦点", Tokens::get(ui).focus),
                ("危险", ui.visuals().error_fg_color),
            ] {
        ...
        Panel::new("01  任务界面").show(ui, |ui| {
            ui.columns(2, |cols| {
                ...
                cols[1].vertical(|ui| {
                    ...
                    ui.label(RichText::new(quest.rank).size(16.0));
                    ui.label(quest.goal);
                    properties(
                        ui,
                        &[
        ...
        ResponsiveColumns::new(Id::new("detail-columns"))
            ...
            .show(ui, 2, |ui, column| {
                if column == 0 {
                    ...
                    self.equipment_details(ui);
                    ui.add_space(6.0);
                    self.guild_records(ui);
                }
            });
    }
    ...
    fn inventory(&mut self, ui: &mut egui::Ui) {
        ...
        Panel::new("02  道具袋").show(ui, |ui| {
            ...
            let size =
                ((ui.available_width() - (columns - 1) as f32 * 8.0) / columns as f32).min(64.0);
    ...
    fn controls(&mut self, ui: &mut egui::Ui) {
        Panel::new("03  基础组件与状态").show(ui, |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                let label = options[self.keycap_preview];
                let label_width = ui
                    .painter()
                    .layout_no_wrap(
                        label.to_owned(),
                        FontSelection::Default
                            .resolve_with_fallback(ui.style(), TextStyle::Button.into()),
                        ui.visuals().text_color(),
                    )
                    .size()
                    .x;
                let width = ui
                    .spacing()
                    .combo_width
                    .max(label_width + ui.spacing().button_padding.x * 2.0 + 24.0);
                let hints = ui
                    .allocate_ui_with_layout(
                        vec2(width, ui.spacing().interact_size.y),
                        egui::Layout::top_down(egui::Align::Min),
                        |ui| ui.add(Button::new(label).id(id).full_width()),
                    )
                    .inner;
                let center = pos2(
                    hints.rect.right() - ui.spacing().button_padding.x - 8.0,
                    hints.rect.center().y,
                );
                ui.painter_at(hints.rect).add(egui::Shape::line(
                    vec![
                        center + vec2(-4.0, -2.0),
                        center + vec2(0.0, 2.0),
                        center + vec2(4.0, -2.0),
                    ],
                    Stroke::new(1.5, ui.style().interact(&hints).text_color()),
                ));
    ...
    fn hud(&mut self, ui: &mut egui::Ui) {
        Panel::new("04  战斗 HUD").show(ui, |ui| {
            ...
            painter.circle_filled(map, 31.0, ui.visuals().faint_bg_color.gamma_multiply(0.6));
    ...
    fn quest_archive(&mut self, ui: &mut egui::Ui, engagement: &mut FocusEngagement) {
        ...
        engagement.show(ui, region, |ui, controls| {
            ...
            panel.scroll = panel.scroll.max_height(258.0).content_margin(egui::Margin {
                right: 12,
                ..egui::Margin::ZERO
            });
