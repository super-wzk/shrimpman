...
fn collect_focus_shapes(shape: &Shape, clip: Rect, active: egui::Color32, frame: &mut Frame) {
    match shape {
        ...
        Shape::Rect(rect) if rect.stroke.color == active && rect.stroke.width == 2.0 => {
            frame.highlights.push((rect.rect, clip));
        }
        _ => {}
    }
}
...
fn assert_highlight(frame: &Frame, target: Rect) {
    assert_eq!(
        ...
        1,
        "only the current part has a focus border"
    );
    let (highlight, clip) = frame.highlights[0];
    assert_eq!(highlight, target);
...
fn keyboard_scrolling_highlights_the_viewport_without_recoloring_its_panel() {
    ...
    assert_eq!(highlights, [viewport]);
    assert_eq!(background, ctx.global_style().visuals.window_fill);
}
