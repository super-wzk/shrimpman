...
fn find_panel(shapes: &[Shape], fill: Color32, stroke: Stroke) -> bool {
    shapes.iter().any(|shape| match shape {
        Shape::Vec(shapes) => find_panel(shapes, fill, stroke),
        Shape::Rect(rect) => {
            rect.fill == fill
                && rect.corner_radius == egui::CornerRadius::same(12)
                && rect.stroke == stroke
        }
        _ => false,
    })
    ...
    for kind in [
        ...
        for inherit in [false, true] {
            ...
            for pass in 0..4 {
                last = Some(frame(&ctx, f64::from(pass) * 0.2, |ui| {
                    ui.scope(|ui| {
                        ...
                        ui.visuals_mut().faint_bg_color = fill;
            ...
            let expected_background =
                if !inherit && matches!(kind, AreaKind::Popup | AreaKind::Tooltip) {
                    theme.style.visuals.faint_bg_color
                } else {
                    expected_fill
                };
            assert_eq!(
                probe.background, expected_background,
            ...
            assert!(
                find_panel(&shapes, expected_background, expected_stroke),
