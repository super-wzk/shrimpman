...
count 位于头部 `+0x1a` 的末表保存 16 字节动画命令。`11395300` 以记录 `+0`
的 u16 序列 ID 选择首个匹配项；`11395430` 从该项顺序前进，按 `+2` 的通道
调用动画读取器，遇到通道 0 终止。工作台仍展示声明范围内的全部物理记录，不丢弃终止项。

| 命令偏移 | 存储类型 | 含义 |
| --- | --- | --- |
| +0 | u16 | `sequence_id`，序列选择 ID |
| +2 | u8 | `channel`，0 为终止，1..11 为下表的通道；其他值原样保留 |
| +3 | u8 | 未确认 |
| +4 | u32 | `sequence_flags`；所选序列起点的 bit 0 控制整段重新开始，其余位保留 |
| +8 | i32 | `sequence_delay`；所选序列起点的延迟步数，`11395160` 按有符号计数处理 |
| +0x0c | u32 | `animation_id`，交给通道读取器匹配目标记录 ID，不能按数组下标解释 |

通道名称来自原生动画类；每个通道读取不同的物理表：

| 通道 | 动画类型 | 头部 count 偏移 | 记录步长 | ID 查找入口 |
| --- | --- | --- | ---: | --- |
| 1 | DirectionalLight | +0x04 | 28 | `11395B30` |
| 2 | CubeMapLight | +0x06 | 40 | `11395BF0` |
| 3 | GodRay | +0x08 | 60 | `11395D70` |
| 4 | HeightFog | +0x0a | 36 | `11395C50` |
| 5 | DistanceFog | +0x0c | 32 | `11395CB0` |
| 6 | DOF | +0x0e | 44 | `11395D10` |
| 7 | Bloom | +0x10 | 28 | `11395DD0` |
| 8 | ShadowMap | +0x12 | 52 | `11395E30` |
| 9 | ToneMapping | +0x14 | 20 | `11395E90` |
| 10 | GaussianBlur | +0x16 | 28 | `11395EF0` |
| 11 | PointLight | +0x1c | 16 | `11395B90` |

这些入口按目标记录 `+0` 的 u16 ID 找到**第一个连续同 ID 段**，遇到另一个 ID
即停止；不合并后面再次出现的同 ID 记录。命令 ID 保留完整 u32，大于 `0xffff`
的值不能匹配目标 u16 ID，不截断为低 16 位。

通道 1..10 的公共前缀为 `u16 animation_id@0`、`u16 frame@2`、
`u8 flags@4` 和三字节 `unknown_05`。动画设置入口 `113960F0`、`11396600`、
`11395610..11395A50`、`113984D0` 消费帧值及 flags 的 bit 0 作为循环条件。
帧和延迟保留原生更新步数，不换算为秒；前缀后的未确认参数继续按原始 DWORD 展示和编辑。

通道 11 使用独立选择记录：`+2` 的 u16 与 `+4` 的 DWORD 仍未知，不能套用帧和
循环字段。`11398B80/11398CF0` 使用 `u16 animation_group_id@8`、
`u16 first_light_id@0x0a`、`u16 last_light_id@0x0c`；点光源范围包含末端，
末端为 `0xffff` 时使用起点。`+0x0e` 的 u16 原样保留。光照动画组 ID 属于光照
管理器的资源身份，不能当成本渲染表中的数组偏移。

工作台命令的“目标动画记录”是指向原表连续段的只读定位字段，真实记录只属于原来的
物理表节点，字段编辑仍写回该节点的原始 buffer/range。未知通道和缺失 ID 明确显示，
不会猜测目标或复制出第二套资源。解析不创建原生播放器。

`tests/stage_render_animation_samples.rs` 可通过 `MHF_RESOURCE_GAME_ROOT` 只读检查
HD 场景资源的命令及目标切片；常规测试覆盖连续段选择、非连续重复 ID、完整 DWORD
引用、通道 11 的不同布局，以及工作台非零基准下的字段编辑与引用定位。
