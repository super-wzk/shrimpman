...
| `100021C0`, `10002220`, `100022A0` | FSKL metadata supplies root node ordinals; node enumeration skips blocks whose kind's low byte is zero; child/sibling links use node ordinals; record `0x10/0x20/0x30` contain four floats each; `0x40/0x44` are read into runtime WORD fields |
...
- The root-index table is not arbitrary zero padding, and node `0x44` is not
  claimed to be an IK chain ID until its use is confirmed.
...
| `0x40/0x44` | Two full unknown u32 fields; runtime uses low WORDs |
